...
def semantic_main(**kwargs):
    ...
    if mode == 'train':
        ...
        for n_iter in range(start_iter, config.max_iteration):
            ...
            gt_label_flatten = gt_label.reshape(-1, )  # [H * W]
            ...
            # display loss
          #  print("Step: {0}. Lr: {1}. Loss: {ssloss.val:.6f} ({loss.avg:.4f})".format(n_iter + 1, lr, loss=losses))
            print(",",losses.val)
        ...
        #print(outputs_up_flatten[34312, :])
        #print(gt_label_flatten.data)
        ...
        for imgIndex in range(1, nImgs + 1):
            ...
            fast= fast_hist(np.squeeze(gt_label).flatten(),
                              pred_label.flatten(),
            ...
            local_acc = np.diag(hist_local).sum() / hist_local.sum()
            #print('>>>local accuracy',local_acc)
            outstr +=str(local_acc)+',' + '\n'
            if mode == 'test':
                test_acc.append(local_acc) 
            if mode == 'val':
                val_acc.append(local_acc)
            iou_local = np.diag(hist_local) / (hist_local.sum(1) + hist_local.sum(0) - np.diag(hist_local))
            mean_iou_local = np.nanmean(iou_local)
            #print('>>> mean  localIoU', mean_iou_local)
            outstr += '>>> mean IoU local ' + str(mean_iou_local) + '\n'
            # IoU of each class
            #print('>>> IoU_local of each class')
            outstr += '\n>>> IoU of each class' + '\n'
            for classIdx in range(nSketchClasses):
                ...
                for ipad in range(cat_max_len - cat_len):
                    pad += ' '
                singlestr += cat_name + pad + str(iou_local[classIdx])
                if iou_local[classIdx] != 0:
                  #print(singlestr)
                  outstr += singlestr + '\n' 
        # ignore bg pixel with val
        if ignore_class_bg:
                hist = hist[1:, 1:]
        if dcrf:
